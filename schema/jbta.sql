-- CREATE USER
CREATE USER "BTM_OWNER" PROFILE "DEFAULT" IDENTIFIED BY "***"
    DEFAULT TABLESPACE "WEBAPPS"
    TEMPORARY TABLESPACE "TEMP"
    QUOTA UNLIMITED ON "WEBAPPS" ACCOUNT UNLOCK;

GRANT CREATE JOB TO "BTM_OWNER";
GRANT CREATE PROCEDURE TO "BTM_OWNER";
GRANT CREATE SEQUENCE TO "BTM_OWNER";
GRANT CREATE SESSION TO "BTM_OWNER";
GRANT CREATE SYNONYM TO "BTM_OWNER";
...
GRANT SELECT ON BTA_OWNER.HALL_AVAILABILITY TO BTM_OWNER;
-- Bridge between old and new
GRANT SELECT ON BTA_OWNER.AVAILABILITY TO BTM_OWNER;
...
CREATE SEQUENCE BTM_OWNER.EXP_HALL_EMAIL_RECIPIENT_ID
    INCREMENT BY 1
    START WITH 1;

CREATE SEQUENCE BTM_OWNER.EXP_HALL_HOUR_ID
    INCREMENT BY 1
    START WITH 1;

CREATE SEQUENCE BTM_OWNER.EXP_HALL_HOUR_REASON_TIME_ID
    INCREMENT BY 1
    START WITH 1;

CREATE SEQUENCE BTM_OWNER.EXP_HALL_SHIFT_ID
    INCREMENT BY 1
    START WITH 1;
...
CREATE SEQUENCE BTM_OWNER.EXP_HALL_SIGNATURE_ID
    INCREMENT BY 1
    START WITH 1;

CREATE SEQUENCE BTM_OWNER.OP_ACC_HOUR_ID;

CREATE SEQUENCE BTM_OWNER.OP_CROSS_CHECK_COMMENT_ID
    INCREMENT BY 1
    START WITH 1
    NOCYCLE
    NOCACHE
...
CREATE SEQUENCE BTM_OWNER.OP_HALL_HOUR_ID
    INCREMENT BY 1
    START WITH 1;

CREATE SEQUENCE BTM_OWNER.OP_MULTIPLICITY_HOUR_ID;

CREATE SEQUENCE BTM_OWNER.OP_SHIFT_ID;

CREATE SEQUENCE BTM_OWNER.OP_SIGNATURE_ID;
...
(
    ...
    CONSTRAINT EXPERIMENTER_SHIFT_FK1 FOREIGN KEY (HALL, PURPOSE_ID) REFERENCES BTM_OWNER.EXP_HALL_SHIFT_PURPOSE (HALL, EXP_HALL_SHIFT_PURPOSE_ID)
);
...
(
    ...
    CONSTRAINT HOUR_REASON_TIME_AUD_FK1 FOREIGN KEY (REV) REFERENCES BTM_OWNER.REVISION_INFO (REV) ON DELETE SET NULL
);
...
(
    ...
    CONSTRAINT R_35 FOREIGN KEY (HALL, EXP_HALL_HOUR_ID) REFERENCES BTM_OWNER.EXP_HALL_HOUR (HALL, EXP_HALL_HOUR_ID) ON DELETE SET NULL,
    CONSTRAINT EXPERIMENTER_HOUR_REASON_FK1 FOREIGN KEY (HALL, REASON) REFERENCES BTM_OWNER.EXP_HALL_REASON (HALL, NAME) ON DELETE CASCADE
);
