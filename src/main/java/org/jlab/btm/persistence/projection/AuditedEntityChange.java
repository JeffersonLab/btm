...
public class AuditedEntityChange {
    ...
    public AuditedEntityChange(long revision, RevisionType type, BigInteger entityId, Class entityClass, Date date, Hall hall) {
        ...
        switch(entityClass.getSimpleName()) {
            case "ExpShift":
                ...
                this.name = hall.name() + " " + shiftFormat.format(date) + " " + BtmTimeUtil.calculateExperimenterShift(date).name();
                break;
            case "CcShift":
                this.url = "/reports/activity-audit/cc-shift?entityId=" + entityId;
                this.name = shiftFormat.format(date) + " " + TimeUtil.calculateCrewChiefShiftType(date).name();
                break;
            case "ExpHour":
