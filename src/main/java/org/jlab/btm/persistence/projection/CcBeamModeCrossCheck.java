...
public class CcBeamModeCrossCheck {
  ...
  public CcBeamModeCrossCheck(
    ...
    for (int i = 0; i < 4; i++) {
      highHallPhysicsMessage[i] =
          "Operations Hall "
              + halls[i]
              + " reports significantly more UP + TUNE + BNR + PHYSICS DOWN ("
              + BtmFunctions.formatDuration(
                  times[i].getUpSeconds()
                      + times[i].getTuneSeconds()
                      + times[i].getBnrSeconds()
                      + times[i].getDownSeconds(),
                  DurationUnits.HOURS)
              + " hours) than the Operations Accelerator reported PHYSICS ("
    ...
    highHallPhysics[0] =
        a.getUpSeconds() + a.getTuneSeconds() + a.getBnrSeconds() + a.getDownSeconds()
            <= accSeconds + TEN_MINUTES_OF_SECONDS;
    highHallPhysics[1] =
        b.getUpSeconds() + b.getTuneSeconds() + b.getBnrSeconds() + b.getDownSeconds()
            <= accSeconds + TEN_MINUTES_OF_SECONDS;
    highHallPhysics[2] =
        c.getUpSeconds() + c.getTuneSeconds() + c.getBnrSeconds() + c.getDownSeconds()
            <= accSeconds + TEN_MINUTES_OF_SECONDS;
    highHallPhysics[3] =
        d.getUpSeconds() + d.getTuneSeconds() + d.getBnrSeconds() + d.getDownSeconds()
            <= accSeconds + TEN_MINUTES_OF_SECONDS;
