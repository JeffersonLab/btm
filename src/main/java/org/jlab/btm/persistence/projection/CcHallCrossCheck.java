...
public class CcHallCrossCheck {
  ...
  public CcHallCrossCheck(
      ...
      CcHallShiftTotals opB,
      CcHallShiftTotals opC,
      CcHallShiftTotals opD,
      ExpShiftTotals a,
      ExpShiftTotals b,
      ExpShiftTotals c,
      ExpShiftTotals d) {
    ...
    for (int i = 0; i < 4; i++) {
      highAbuMessage[i] =
          "Experimenter Hall "
              ...
              + BtmFunctions.formatDuration(
                  times[i].getAbuSeconds() + times[i].getBanuSeconds(), DurationUnits.HOURS)
              + " hours) than the Operations reported UP + TUNE + BNR ("
              + BtmFunctions.formatDuration(
                  opTimes[i].getUpSeconds()
                      + opTimes[i].getTuneSeconds()
      ...
      lowBanuMessage[i] =
          "Experimenter Hall "
              ...
              + " hours) vs the Operations reported BNR ("
              + BtmFunctions.formatDuration(opTimes[i].getBnrSeconds(), DurationUnits.HOURS)
              + " hours)";
      highBnaMessage[i] =
          "Experimenter Hall "
              + halls[i]
    ...
    highAbu[0] =
        ((a.getAbuSeconds() + a.getBanuSeconds()) / 2
                <= opA.getUpSeconds() + opA.getTuneSeconds() + opA.getBnrSeconds())
            || (opA.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
    highAbu[1] =
        ((b.getAbuSeconds() + b.getBanuSeconds()) / 2
                <= opB.getUpSeconds() + opB.getTuneSeconds() + opB.getBnrSeconds())
            || (opB.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
    highAbu[2] =
        ((c.getAbuSeconds() + c.getBanuSeconds()) / 2
                <= opC.getUpSeconds() + opC.getTuneSeconds() + opC.getBnrSeconds())
            || (opC.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
    highAbu[3] =
        ((d.getAbuSeconds() + d.getBanuSeconds()) / 2
                <= opD.getUpSeconds() + opD.getTuneSeconds() + opD.getBnrSeconds())
            || (opD.getUpSeconds() <= HALF_HOUR_OF_SECONDS);

    lowAbu[0] =
        (a.getAbuSeconds() + a.getBanuSeconds()
                >= (opA.getUpSeconds() + opA.getTuneSeconds() + opA.getBnrSeconds()) / 2)
            || (opA.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
    lowAbu[1] =
        (b.getAbuSeconds() + b.getBanuSeconds()
                >= (opB.getUpSeconds() + opB.getTuneSeconds() + opB.getBnrSeconds()) / 2)
            || (opB.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
    lowAbu[2] =
        (c.getAbuSeconds() + c.getBanuSeconds()
                >= (opC.getUpSeconds() + opC.getTuneSeconds() + opC.getBnrSeconds()) / 2)
            || (opC.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
    lowAbu[3] =
        (d.getAbuSeconds() + d.getBanuSeconds()
                >= (opD.getUpSeconds() + opD.getTuneSeconds() + opD.getBnrSeconds()) / 2)
            || (opD.getUpSeconds() <= HALF_HOUR_OF_SECONDS);
