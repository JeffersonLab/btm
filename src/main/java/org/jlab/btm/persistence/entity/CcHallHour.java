...
@Table(
    name = "CC_HALL_HOUR",
    schema = "BTM_OWNER",
    uniqueConstraints = {@UniqueConstraint(columnNames = {"HALL", "DAY_AND_HOUR"})})
@NamedNativeQueries({
  @NamedNativeQuery(
      name = "OpHallHour.insertNATIVE",
      query =
          "INSERT into CC_HALL_HOUR (CC_HALL_HOUR_ID, HALL, DAY_AND_HOUR, UP_SECONDS, TUNE_SECONDS, BNR_SECONDS, DOWN_SECONDS, OFF_SECONDS) values (:id, :hall, to_timestamp_tz(:dayAndHour, 'YYYY-MM-DD HH24 TZD'), :up, :tune, :bnr, :down, :off)",
      resultClass = CcHallHour.class)
})
public class CcHallHour implements Serializable, HourEntity {
  ...
  @NotNull
  @Column(name = "CC_HALL_HOUR_ID", nullable = false, precision = 38, scale = 0)
