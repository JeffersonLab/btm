...
@NamedQueries({
  @NamedQuery(
      name = "ExpHour.findByExpHourId",
      query = "SELECT e FROM ExpHour e WHERE e.expHourId = :expHallHourId")
})
@NamedNativeQueries({
  @NamedNativeQuery(
      name = "ExpHour.findByHallAndDayAndHourNATIVE",
      query =
          "SELECT * FROM EXP_HOUR WHERE hall = :hall AND to_char(DAY_AND_HOUR, 'YYYY-MM-DD HH24 TZD') = :dayAndHour",
...
public class ExpHour extends HallHour {
  ...
  @Column(
      ...
      insertable = false,
      updatable = false,
      columnDefinition = "timestamp(0) with local timezone")
  ...
  /**
   * Copies the time accounting data plus the remark from the specified experimenter hall hour into
   * this one.
   *
   ...
   *
   * <p>The accelerator mutual exclusive set consists of ABU, BANU, BNA, ACC, and OFF.
   *
   * @return the sum.
   */
   ...
   *
   * <p>The experimenter mutual exclusive set consists of ER, PCC, UED, and OFF.
   *
   * @return the sum.
   */
