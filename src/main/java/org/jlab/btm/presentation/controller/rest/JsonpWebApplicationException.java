...
public class JsonpWebApplicationException extends WebApplicationException {
    ...
    private static Response createJsonPResponse(Response.Status status, String message, String callback) {
        JsonObjectBuilder builder = Json.createObjectBuilder().add("message", ObjectUtil.coalesce(message, ""));
        return Response.status(status).entity(callback + "(" + builder.build().toString() + ");").build();
    }
}
