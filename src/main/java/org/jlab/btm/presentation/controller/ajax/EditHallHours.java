...
@WebServlet(
    name = "EditHallHours",
    urlPatterns = {"/ajax/edit-hall-hours"})
public class EditHallHours extends HttpServlet {

  private static final Logger logger = Logger.getLogger(EditHallHours.class.getName());
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      Short[] tuneArray = ParamConverter.convertShortArray(request, "tune[]", (short) 0);
      Short[] bnrArray = ParamConverter.convertShortArray(request, "bnr[]", (short) 0);
      ...
      hallHourService.editHallHours(
          hall, hourArray, upArray, tuneArray, bnrArray, downArray, offArray);
      ...
      logger.log(
          Level.WARNING, "Unable to edit hall availability hours due to access exception", e);
      errorReason = "Access Denied";
    } catch (UserFriendlyException e) {
      logger.log(Level.INFO, "Unable to edit hall availability hours: {0}", e.getMessage());
      errorReason = e.getMessage();
    } catch (Exception e) {
      Throwable rootCause = ExceptionUtil.getRootCause(e);
      if (rootCause instanceof SQLException) {
        errorReason = "Database exception";
        logger.log(Level.SEVERE, "Unable to edit hall hours", e);
        ...
        logger.log(Level.FINEST, "Unable to edit hall hours due to constraint violoation", e);
