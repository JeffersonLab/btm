...
public class WeekSummary {
    ...
    public Response getSummaryJson(
            @QueryParam("week-start") final String ccShiftStart
    ) {
            ...
            public void write(OutputStream out) {
                try (JsonGenerator gen = Json.createGenerator(out)) {
                    ...
                    gen.writeStartObject()
                            .write("acc-physics-seconds", ObjectUtil.coalesce(
                                    accTotals.getUpSeconds(), 0L))
                            .write("acc-studies-seconds", ObjectUtil.coalesce(
                                    accTotals.getStudiesSeconds(), 0L))
                            .write("acc-restore-seconds", ObjectUtil.coalesce(
                                    accTotals.getRestoreSeconds(), 0L))
                            .write("acc-acc-seconds", ObjectUtil.coalesce(accTotals.getAccSeconds(),
                                    0L))
                            .write("acc-down-seconds", ObjectUtil.coalesce(
                                    accTotals.getDownSeconds(), 0L))
                            .write("acc-sad-seconds", ObjectUtil.coalesce(accTotals.getSadSeconds(),
                                    0L))
                            .write("plan-studies-seconds", ObjectUtil.coalesce(
                                    accScheduledMap.get("studies"), 0L))
                            .write("plan-restore-seconds", ObjectUtil.coalesce(
                                    accScheduledMap.get("restore"), 0L))
                            .write("plan-acc-seconds", ObjectUtil.coalesce(
                                    accScheduledMap.get("acc"), 0L))
                    ...
                    for (ExpHallHourTotals totals : expHallHourTotals) {
                        ...
                        gen.writeStartObject()
                                ...
                                .write("pd-scheduled-seconds", ObjectUtil.coalesce(hallScheduledMap.get(
                                        totals.getHall()), 0L))
                                ...
                                .write("banu-seconds", ObjectUtil.coalesce(
                                        ...
                                        0))
                                .write("acc-seconds", ObjectUtil.coalesce(
                                        totals.getAccSeconds(),
                                        0))
                                .write("er-seconds", ObjectUtil.coalesce(
                                        totals.getErSeconds(),
                                        0))
                                .write("pcc-seconds", ObjectUtil.coalesce(
                                        totals.getPccSeconds(),
                                        0))
                                .write("ued-seconds", ObjectUtil.coalesce(
                                        totals.getUedSeconds(),
                                        0))
                                .writeEnd();
                    }
