 ...
 * @author ryans
 */
@WebServlet(name = "EditProgram", urlPatterns = {"/ajax/edit-program"})
public class EditProgram extends HttpServlet {

    private static final Logger logger = Logger.getLogger(
            EditProgram.class.getName());
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            purposeService.edit(programId, name, alias, url, experiment, active);
        } catch (EJBAccessException e) {
            logger.log(Level.WARNING,
                    "Unable to edit program due to access exception", e);
            errorReason = "Access Denied";
        } catch (UserFriendlyException e) {
            logger.log(Level.INFO,
                    "Unable to edit program: {0}",
                    e.getMessage());
            errorReason = e.getMessage();
        } catch (Exception e) {
            logger.log(Level.SEVERE,
                    "Unable to edit program", e);
