...
public class ShiftSummary {
    ...
    public Response getSummaryJson(
            @QueryParam("shift-start") final String ccShiftStart
    ) {
            ...
            public void write(OutputStream out) {
                try (JsonGenerator gen = Json.createGenerator(out)) {
                    ...
                    try {
                        ...
                        throw new JsonWebApplicationException(Response.Status.BAD_REQUEST,
                    ...
                    if (!TimeUtil.isCrewChiefShiftStart(ccStart)) {
                        throw new JsonWebApplicationException(Response.Status.BAD_REQUEST,
                                "shift start hour must be one of 23, 7, 15");
                    }
                    ...
                    gen.writeStartObject()
                            ...
                            .write("acc-physics-seconds", ObjectUtil.coalesce(accAvail.getShiftTotals().getUpSeconds(), 0))
                            .write("acc-studies-seconds", ObjectUtil.coalesce(accAvail.getShiftTotals().getStudiesSeconds(), 0))
                            .write("acc-restore-seconds", ObjectUtil.coalesce(accAvail.getShiftTotals().getRestoreSeconds(), 0))
                            .write("acc-acc-seconds", ObjectUtil.coalesce(accAvail.getShiftTotals().getAccSeconds(), 0))
                            .write("acc-down-seconds", ObjectUtil.coalesce(accAvail.getShiftTotals().getDownSeconds(), 0))
                            .write("acc-sad-seconds", ObjectUtil.coalesce(accAvail.getShiftTotals().getSadSeconds(), 0))
                            .write("plan-physics-seconds", ObjectUtil.coalesce(accAvail.getPdShiftTotals().getUpSeconds(), 0))
                            .write("plan-studies-seconds", ObjectUtil.coalesce(accAvail.getPdShiftTotals().getStudiesSeconds(), 0))
                            .write("plan-restore-seconds", ObjectUtil.coalesce(accAvail.getPdShiftTotals().getRestoreSeconds(), 0))
                            .write("plan-acc-seconds", ObjectUtil.coalesce(accAvail.getPdShiftTotals().getAccSeconds(), 0))
                            .write("plan-down-seconds", ObjectUtil.coalesce(accAvail.getPdShiftTotals().getDownSeconds(), 0))
                    ...
                    for (ExpHallShiftTotals totals : expHourList) {
                        ...
                        gen.writeStartObject()
                                ...
                                .write("pd-scheduled-seconds", ObjectUtil.coalesce(scheduleMap.get(
                                        totals.getHall()), 0))
                                .write("abu-seconds", ObjectUtil.coalesce(
                                        totals.getAbuSeconds(), 0))
                                .write("banu-seconds", ObjectUtil.coalesce(
