...
public class MonthlyScheduleService extends AbstractService<MonthlySchedule> {
  ...
  private static final Logger LOGGER = Logger.getLogger(MonthlyScheduleService.class.getName());
  ...
  /**
   * Find most recent published schedule for the month given the first day of the month. If none
   * published, then return most recent tentative schedule for the month.
   *
  ...
  public MonthlySchedule findMostRecentPublished(List<MonthlySchedule> scheduleList) {
    ...
    if (scheduleList != null && !scheduleList.isEmpty()) {
      ...
      if (schedule == null) {
        for (MonthlySchedule s : scheduleList) {
          if (schedule == null || schedule.getVersion() < s.getVersion()) {
            schedule = s;
          }
        }
      }
  ...
  /**
   * Find the most recently published set of monthly schedules that are included with the specified
   * date range. For any given month if no schedules are published then them most recent tentative
   * schedule is used, or if none at all then no record is included for that month.
   *
  ...
  public void editRow(
      ...
      boolean hallBPolarized,
      boolean hallCPolarized,
      boolean hallDPolarized,
      Integer hallAPasses,
      Integer hallBPasses,
      Integer hallCPasses,
      Integer hallDPasses,
    ...
    if (schedule == null) {
      throw new UserFriendlyException(
          "Unable to find monthly schedule for supplied ID: " + scheduleId);
    }
  ...
  private void editRow(
    ...
    ScheduleDay scheduleDay =
        scheduleDayService.findByDateAndSchedule(date, schedule.getMonthlyScheduleId());
  ...
  @PermitAll
  public List<ScheduleDay> filterScheduleDaysFromRange(
      List<MonthlySchedule> monthlySchedules, Date start, Date end) {
  ...
  private Date findPreviousOff(Date today, MonthlySchedule currentMonth, boolean inverse) {
    ...
    if (previousOff == null) {
      ...
      if (Duration.between(today.toInstant(), startOfPreviousMonth.toInstant())
              .compareTo(Duration.ofDays(365))
          < 0) {
        MonthlySchedule schedule = findMostRecentPublished(startOfPreviousMonth);
        if (schedule
            != null) { // If previous month has no schedule at all then just return null, let's bail
          // as months with no schedule at all are simply not supported!
  ...
  public DateRange getCurrentRunBounds() {
    ...
    Date today = new Date();
    MonthlySchedule schedule =
        findMostRecentPublished(TimeUtil.startOfMonth(today, Calendar.getInstance()));
    if (schedule != null) {
  ...
  public DateRange getPreviousRunBounds(Date previousToDate) {
    DateRange bounds = null;
    MonthlySchedule schedule =
        findMostRecentPublished(TimeUtil.startOfMonth(previousToDate, Calendar.getInstance()));
  ...
  public PacAccSum findSummary(Date start, Date end) {
    ...
    start = TimeUtil.startOfDay(start, Calendar.getInstance());
        ...
        BtmTimeUtil.isStartOfDay(end) ? end : TimeUtil.startOfNextDay(end, Calendar.getInstance());
    ...
    List<MonthlySchedule> monthlySchedules =
        this.findMostRecentPublishedInDateRange(start, inclusiveEnd);
    List<ScheduleDay> scheduleDays =
        this.filterScheduleDaysFromRange(monthlySchedules, start, inclusiveEnd);
    ...
    for (ScheduleDay day : scheduleDays) {
      if (!"OFF".equals(day.getAccProgram())) {
        ...
        record.offDays++; // May be implied off gaps that are missed so this number is explicit off
        // only...
      }
    }
