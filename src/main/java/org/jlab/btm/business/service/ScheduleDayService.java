...
public class ScheduleDayService extends AbstractService<ScheduleDay> {
    ...
    public List<ScheduleDay> find(BigInteger monthlyScheduleId, Date start, Date end) {
        ...
        /**
         * JPA won't create object for me via getResultList due to day_month_year coming from dual
         * table so I use JPAUtil reflection, but it only works on objects with a single special
         * constructor and the entity must have a default no-arg constructor so we cannot use
         * ScheduleDay directly and instead use a ProjectedScheduleDay.
         */
        //List<ScheduleDay> scheduleList = q.getResultList();
        List<ProjectedScheduleDay> scheduleList = JPAUtil.getResultList(q,
                ProjectedScheduleDay.class);
    ...
    public ScheduleDay findByDateAndSchedule(Date dayMonthYear, BigInteger monthlyScheduleId) {
        TypedQuery<ScheduleDay> q = em.createQuery(
