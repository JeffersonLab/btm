...
public class CcAccHourAudService extends AbstractService<CcAccHourAud> {
    ...
    @PermitAll
    public List<CcAccHourAud> filterList(BigInteger entityId, BigInteger revisionId, int offset, int max) {
        ...
        List<CcAccHourAud> entityList = getEntityManager().createQuery(cq).setFirstResult(offset).setMaxResults(max).getResultList();
