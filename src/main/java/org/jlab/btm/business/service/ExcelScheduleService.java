...
public class ExcelScheduleService {
  ...
  @EJB ExpProgramService purposeService;
  @EJB MonthlyScheduleService scheduleService;

  @RolesAllowed({"schcom", "btm-admin"})
  public List<MonthlySchedule> upload(InputStream in)
      throws IOException, InvalidFormatException, UserFriendlyException {
    List<MonthlySchedule> resultList = new ArrayList<>();

    Workbook wb =
        WorkbookFactory.create(
            in); // Wrap in try with resources doesn't work as close method results in
    // NoSuchMethodError?
    ...
    if (hallAPurposeMap.get("OFF") == null) {
      throw new UserFriendlyException(
          "Hall A must have a program named 'OFF', please update program list");
    }

    if (hallBPurposeMap.get("OFF") == null) {
      throw new UserFriendlyException(
          "Hall B must have a program named 'OFF', please update program list");
    }

    if (hallCPurposeMap.get("OFF") == null) {
      throw new UserFriendlyException(
          "Hall C must have a program named 'OFF', please update program list");
    }

    if (hallDPurposeMap.get("OFF") == null) {
      throw new UserFriendlyException(
          "Hall D must have a program named 'OFF', please update program list");
    }
        ...
        for (int i = 0; i < 4; i++) {
          try {
            passes[i] =
                Math.round(
                    Float.valueOf(
                        tokens[i])); // Hall D might read 0.5 or 5.5, so round up and GUI will know
            // what to do.
  ...
  private HallProperties parseHallProperties(Cell cell, Hall hall, String currentUnits)
      ...
      if (hallProperties != null && !hallProperties.trim().isEmpty()) {
        ...
        if (tokens.length != 4) {
          throw new UserFriendlyException(
              "Hall "
                  + hall
                  + " Properties must be formated 'GeV/mA/Pol/MHz', found: "
                  + hallProperties);
        }
        ...
        try {
          ...
          throw new UserFriendlyException(
              "Hall " + hall + " GeV must be a number, found: " + tokens[0]);
        }
        ...
        try {
          ...
          throw new UserFriendlyException(
              "Hall " + hall + " current must be a number, found: " + tokens[1]);
        }
        ...
        if ("p".equalsIgnoreCase(tokens[2])) {
          ...
          throw new UserFriendlyException(
              "Hall " + hall + " Pol must be one of 'p,-', found: " + tokens[2]);
        }
        ...
        try {
          ...
          throw new UserFriendlyException(
              "Hall " + hall + " MHz must be a number, found: " + tokens[3]);
        }
      }
    }
  ...
  private Integer parseHallProgram(Cell cell, Map<String, ExpProgram> purposeMap, Hall hall)
      throws UserFriendlyException {
  ...
  @PermitAll
  public void export(
      OutputStream out, MonthlySchedule schedule, Map<Integer, ExpProgram> purposeMap)
      throws IOException {
    ...
    String headerValue =
        monthDateFormat.format(schedule.getStartDay()) + "; Version " + schedule.getVersion();

    if (schedule.getPublishedDate() != null) {
      headerValue =
          headerValue
              + " (Published "
              + monthAndDayDateFormat.format(schedule.getPublishedDate())
              + ")";
    } else {
    ...
    for (ScheduleDay day : schedule.getScheduleDayList()) {
      ...
      row.createCell(16)
          .setCellValue(day.getHallAPolarized() == null ? "" : day.getHallAPolarized() ? "✔" : "");
      row.createCell(17)
          .setCellValue(day.getHallBPolarized() == null ? "" : day.getHallBPolarized() ? "✔" : "");
      row.createCell(18)
          .setCellValue(day.getHallCPolarized() == null ? "" : day.getHallCPolarized() ? "✔" : "");
      row.createCell(19)
          .setCellValue(day.getHallDPolarized() == null ? "" : day.getHallDPolarized() ? "✔" : "");
      c = row.createCell(20);
      c.setCellStyle(intStyle);
      ...
      row.createCell(24)
          .setCellValue(
              BtmFunctions.formatPriority(
                  day.getHallAPriority(),
                  day.getHallBPriority(),
                  day.getHallCPriority(),
                  day.getHallDPriority(),
                  day.getHallAKiloVolts(),
                  day.getHallBKiloVolts(),
                  day.getHallCKiloVolts(),
                  day.getHallDKiloVolts()));
