...
public class ExpHourService extends AbstractService<ExpHour> {
  ...
  /**
   * WARNING WARNING WARING: "end" is INCLUSIVE here; unlike pretty much everywhere else
   *
   * @param start
   * @param end
   * @return
  ...
  @PermitAll
  public ExpShiftAvailability getHallAvailability(
      Hall hall, Date startHour, Date endHour, boolean queryEpics) {
    List<ExpHour> dbHourList = findInDatabase(hall, startHour, endHour);
  ...
  @PermitAll
  public List<ExpHour> fillMissingHoursAndSetSource(
      Map<Date, ExpHour> dbHourMap,
      Map<Date, ExpHour> epicsHourMap,
      Hall hall,
      Date start,
      Date end) {
  ...
  /**
   * Fetch the experimenter hall hours for the specified hall, start day and hour, and end
   * dayAndHour from the database.
   *
   * @param hall the hall.
  ...
  public List<ExpHour> findInDatabase(Hall hall, Date startDayAndHour, Date endDayAndHour) {

    // We must use to_timestamp_tz in query because if you rely on JPA criteria query it will not
    // include TZ Offset,
    // which means you couldn't select between the two wall clock ambiguous daylight savings hours
    Query query =
        em.createNativeQuery(
            "select * from EXP_HOUR a where a.hall = :hall and a.day_and_hour between to_timestamp_tz(:start, 'YYYY-MM-DD HH24 TZD') and to_timestamp_tz(:end, 'YYYY-MM-DD HH24 TZD')",
            ExpHour.class);
    ...
    logger.log(
        Level.FINEST,
        "ExpHallHourFacade.findInDatabase: {0} - {1}",
        new Object[] {startDayAndHour, endDayAndHour});
  ...
  public ExpHour findInDatabase(Hall hall, Date dayAndHour) {
    ...
    Query query =
        em.createNativeQuery(
            "select * from EXP_HOUR a where a.hall = :hall and a.day_and_hour = to_timestamp_tz(:dayAndHour, 'YYYY-MM-DD HH24 TZD')",
            ExpHour.class);
  ...
  /**
   * Fetch the experimenter hall hours for the specified hall, start day and hour, and end day and
   * hour from EPICS, optionally rounded.
   *
   * @param hall the hall.
   * @param startDayAndHour the start day and hour.
   * @param endDayAndHour the end day and hour.
    ...
    for (int i = 0; i < hourArray.length; i++) {
      ...
      if (total != 3600) {
        ...
        throw new UserFriendlyException(
            "Hour " + hourFormat.format(hourArray[i]) + " accelerator beam time must total 1 hour");
      }
      ...
      if (total != 3600) {
        SimpleDateFormat hourFormat = new SimpleDateFormat("HH");
        throw new UserFriendlyException(
            "Hour " + hourFormat.format(hourArray[i]) + " experiment beam time must total 1 hour");
      }
