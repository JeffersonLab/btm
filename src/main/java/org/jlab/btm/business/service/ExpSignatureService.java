...
public class ExpSignatureService extends AbstractService<ExpSignature> {
  ...
  @EJB ExpShiftService shiftService;
  ...
  public List<ExpSignature> find(Hall hall, Date startDayAndHour) {
    TypedQuery<ExpSignature> q =
        em.createNamedQuery("ExpSignature.findByHallStartDayAndHour", ExpSignature.class);
  ...
  public ExpTimesheetStatus calculateStatus(Hall hall, Date startDayAndHour) {
    ...
    List<ExpHour> availabilityList =
        hourService.findInDatabase(hall, startDayAndHour, endDayAndHour);

    List<ExpUedExplanation> explanationsList =
        explanationService.find(hall, startDayAndHour, endDayAndHour);
  ...
  @PermitAll
  public ExpTimesheetStatus calculateStatus(
      Hall hall,
      Date startDayAndHour,
      Date endDayAndHour,
      List<ExpHour> expAvailabilityList,
      List<ExpUedExplanation> reasonsNotReadyList,
      ExpShift shiftInfo,
    ...
    if (expAvailabilityList != null) {
      if (expAvailabilityList.size() == hoursInShift) {
        ...
        // If count of hours is 1 less than entire shift worth AND last hour in ordered list is NOT
        // equal to last hour of shift then missing hour MUST be from last hour of shift
      } else if (expAvailabilityList.size() == (hoursInShift - 1)
          && !expAvailabilityList
              .get(expAvailabilityList.size() - 1)
              .getDayAndHour()
              .equals(endDayAndHour)) {
        status.setCcHoursComplete(true);
      }
    }
    ...
    List<HourReasonDiscrepancy> discrepancies =
        explanationService.validateUED(expAvailabilityList, reasonsNotReadyList);
    ...
    for (ExpSignature sig : signatureList) {
      ...
      if (sig.getSignedRole() == role) {
        throw new UserFriendlyException(
            "A " + role.getLabel() + " has already signed the timesheet");
      }
    }
  ...
  public void populateRequestAttributes(
      HttpServletRequest request, Hall hall, Date startHour, Date endHour) {
    /*AVAILABILITY*/
    ExpShiftAvailability expAvailability =
        hourService.getHallAvailability(hall, startHour, endHour, true);
