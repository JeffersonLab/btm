...
public class RevisionInfoService extends AbstractService<RevisionInfo> {
  ...
  private void addCommonFilters(
      ...
      Root<RevisionInfo> root,
      ActivityAuditParams params,
    ...
    if (params.getType() != null) {
      ...
      if (params.getType() == TimesheetType.CC) {
        if (params.getShift() != null && params.getTimesheetDate() != null) {
          startHour =
              TimeUtil.getCrewChiefStartDayAndHour(params.getTimesheetDate(), params.getShift());
          endHour =
              TimeUtil.getCrewChiefEndDayAndHour(params.getTimesheetDate(), params.getShift());
        }
        ...
        if (startHour != null) {
          accHourSubquery.where(
              cb.and(cb.greaterThanOrEqualTo(accHourRoot.get("dayAndHour"), startHour)),
              cb.lessThanOrEqualTo(accHourRoot.get("dayAndHour"), endHour));
        }
        ...
        if (params.getShift() != null && params.getTimesheetDate() != null) {
          startHour =
              BtmTimeUtil.getExperimenterStartDayAndHour(
                  params.getTimesheetDate(), params.getShift());
          endHour =
              BtmTimeUtil.getExperimenterEndDayAndHour(
                  params.getTimesheetDate(), params.getShift());
        }
        ...
        if (startHour != null) {
          shiftWherePredicate =
              cb.and(shiftWherePredicate, cb.equal(shiftRoot.get("startDayAndHour"), startHour));
        }
        ...
        if (startHour != null) {
          hourWherePredicate =
              cb.and(
                  hourWherePredicate,
                  cb.and(cb.greaterThanOrEqualTo(hourRoot.get("dayAndHour"), startHour)),
                  cb.lessThanOrEqualTo(hourRoot.get("dayAndHour"), endHour));
        }
  ...
  public List<RevisionInfo> filterList(ActivityAuditParams params) {
    ...
    List<RevisionInfo> revisionList =
        getEntityManager()
            ...
            .setMaxResults(params.getMaxPerPage())
            .getResultList();
  ...
  public List<AuditedEntityChange> findEntityChangeList(long revision) {
    // We drop TZD on to_char because ExpShift doesn't have one, and we can live with ambiguous hour
