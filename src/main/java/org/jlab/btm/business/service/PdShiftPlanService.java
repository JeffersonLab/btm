...
public class PdShiftPlanService extends AbstractService<PdShiftPlan> {
    ...
    public PdShiftPlan findInDatabase(Date startDayAndHour) {
        TypedQuery<PdShiftPlan> query = em.createQuery(
    ...
    /**
     * Returns an array of hall A - D seconds of scheduled time between the start and end dates.
     * Note that shifts cannot be split so if start and end don't fall on crew chief shift
     * boundaries then the data is inaccurate.
     * <p>
     * Another thing to watch out for is the the shift_plans table has a shift column that is
